...
public class Movie {
	...
	private int id;
	private double note;
	public Movie(final JSONObject jsonmovie, final ImageView imageView,final Activity act,final TextView view) {
		try {
			...
			try {
				...
				if(!path2.equals("null"))
					...
					affiche = BitmapFactory.decodeResource(act.getResources(), R.drawable.ic_launcher);
			} catch (Exception e) {
				...
				/*new Thread(new Runnable() {
					public void run() {
						...
						try {
							affiche = ret.retrieveBitmap("http://image.tmdb.org/t/p/w500"+jsonmovie.getString("poster_path"));
	...
	public String toString() {
		return "Titre "+name+" et date "+date+ "et id "+id+" et note"+note;
	}
